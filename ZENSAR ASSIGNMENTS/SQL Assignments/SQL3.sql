...
use ash1
select *from EMP
create or alter function Bonus(@Empjob varchar(10))
returns table
as
return (select Job,Salary,Empname,Deptno from EMP where job=@empjob)
go
update Emp set Salary=Salary+(Salary*0.15) where Deptno=10
update EMP set Salary=Salary+(Salary*0.2) where Deptno=20
update EMP set Salary=Salary+(Salary*0.05) where Deptno<>10 and Deptno<>20

---execution

select *from Bonus('CLERK')
